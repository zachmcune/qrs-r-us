...
function syncLogoOverlay() {
  ...
  logoEditor.sync(readConfigFromForm(form, logoPreviewSrc, logoId), lastCanvas, lastQrRegion);
}

function setupOverlaySync() {
  window.addEventListener("resize", syncLogoOverlay);
  if (typeof ResizeObserver !== "undefined") {
    const observer = new ResizeObserver(() => requestAnimationFrame(syncLogoOverlay));
